...
namespace MFramework.Runtime
{
    ...
    partial class Entity
    {
        ...
        /// 组件对象池字典
        /// </summary>
        private static Dictionary<Type, ObjectPool<EntityComponent>> _comPoolDic = new Dictionary<Type, ObjectPool<EntityComponent>>();
        /// <summary>
        /// 组件类型依赖的类型
        /// </summary>
        ...
        {
            Type type = typeof(T);
            ObjectPool<EntityComponent> pool = null;
            if (!_comPoolDic.TryGetValue(type, out pool))
            {
                pool = new ObjectPool<EntityComponent>();
                pool.onCreate += () => new T();
