    ...
    public abstract class PromiseYieldInstruction : CustomYieldInstruction, IDisposable
    {
        ...
        public void GetResult()
        {
            ...
            switch (_state)
            {
                ...
                default:
                {
                    throw new PromiseInvalidOperationException("Promise is still pending. You must wait for the promise to settle before calling GetResult.", Internal.GetFormattedStacktrace(1));
                }
            }
        }
        ...
        private protected void ValidateOperation()
        {
            if (_retainCounter == 0)
            {
                throw new PromiseInvalidOperationException("Promise yield instruction is not valid after you have disposed. You can get a valid yield instruction by calling promise.ToYieldInstruction().", Internal.GetFormattedStacktrace(1));
            }
        }
    }
