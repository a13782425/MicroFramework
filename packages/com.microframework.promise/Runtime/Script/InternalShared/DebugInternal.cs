...
namespace MFramework.Promise
{
    partial class Internal
    {
        ...
        internal static void ValidateProgressValue(double value, string argName, int skipFrames)
        {
            ...
            if (!isBetween01)
            {
                throw new PromiseArgumentOutOfRangeException(argName, "Must be between 0 and 1. Actual: " + value, GetFormattedStacktrace(skipFrames + 1));
            }
        }
        ...
        internal static void ValidateArgument<TArg>(TArg arg, string argName, int skipFrames)
        {
            if (arg == null)
            {
                throw new PromiseArgumentNullException(argName, null, GetFormattedStacktrace(skipFrames + 1));
            }
        }
        internal static void ValidateArgument(Promise arg, string argName, int skipFrames)
        {
            if (!arg.IsValid)
            {
                throw new PromiseInvalidArgumentException(argName,
        ...
        internal static void ValidateOperation(Promise promise, int skipFrames)
        {
            if (!promise.IsValid)
            {
                throw new PromiseInvalidOperationException("Promise is invalid." +
        ...
        partial class PromiseRefBase
        {
            ...
            private void ValidateAwait(PromiseRefBase other, short promiseId, bool awaited)
            {
                if (new Promise(other, promiseId).IsValid == false)
                {
                    ...
                    if (awaited)
                        throw new PromiseInvalidOperationException("An invalid promise was awaited.", string.Empty);
                    throw new PromiseInvalidReturnException("An invalid promise was returned.", string.Empty);
                }
                ...
                if (other == this)
                {
                    ...
                    if (awaited)
                        throw new PromiseInvalidOperationException("A Promise cannot wait on itself.", string.Empty);
                    throw new PromiseInvalidReturnException("A Promise cannot wait on itself.", string.Empty);
                }
                if (other == null)
                {
                    return;
            ...
            Repeat:
                for (; prev != null; prev = prev._previous)
                {
                    if (prev == this)
                    {
                        ...
                        previouses.Clear();
                        if (awaited)
                            throw new PromiseInvalidOperationException("Circular Promise chain detected.", GetFormattedStacktrace(other));
                        throw new PromiseInvalidReturnException("Circular Promise chain detected.", GetFormattedStacktrace(other));
                    }
    ...
    partial struct Promise
    {
        ...
        static partial void ValidateElement(Promise promise, string argName, int skipFrames)
        {
            if (!promise.IsValid)
            {
                throw new PromiseInvalidElementException(argName,
