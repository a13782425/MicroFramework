...
namespace MFramework.Core.Promise
{
    partial class Internal
    {
        ...
        internal static void ThrowInvalidRaceGroup(int skipFrames)
            => throw new PromiseInvalidOperationException("The promise race group is invalid.", GetFormattedStacktrace(skipFrames + 1));

        [MethodImpl(MethodImplOptions.NoInlining)]
        internal static void ThrowAtLeastOneRaceGroup(int skipFrames)
            => throw new PromiseInvalidOperationException("The promise race group must have at least one promise added to it.", GetFormattedStacktrace(skipFrames + 1));
    } // class Internal
}