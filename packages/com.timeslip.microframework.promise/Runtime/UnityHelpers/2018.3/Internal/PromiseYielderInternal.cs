...
namespace MFramework.Core.Promise
{
    internal static partial class InternalHelper
    {
        ...
        internal static void ValidateIsOnMainThread(int skipFrames)
        {
            if (!IsOnMainThread())
            {
                throw new PromiseInvalidOperationException("Must be on main thread to use PromiseYielder awaits. Use `Promise.SwitchToForeground()` to switch to the main thread.", Internal.GetFormattedStacktrace(skipFrames + 1));
            }
        }
#else
        [MethodImpl(Internal.InlineOption)]
        ...
        internal sealed class YieldInstructionRunner : Internal.HandleablePromiseBase, IEnumerator
        {
            ...
            private void OnCancel(MonoBehaviour runner)
            {
                ...
                Promise.Run((this, runner, _id), (cv) => cv.Item1.OnCancel(cv.runner, cv._id),
                     MicroContext.mainSynchronizationContext) // Explicitly use the sync context in case the user overwrites the Config.ForegroundContext.
                    .Forget();
            }
