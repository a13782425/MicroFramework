...
namespace MFramework.Core.Promise
{
    ...
    public readonly struct CancelationSource : ICancelable, IDisposable, IEquatable<CancelationSource>
    {
        ...
        public void Cancel()
        {
            if (!TryCancel())
            {
                throw new PromiseInvalidOperationException("CancelationSource.Cancel: source is not valid or was already canceled.", Internal.GetFormattedStacktrace(1));
            }
        }
        ...
        public void Dispose()
        {
            if (!TryDispose())
            {
                throw new PromiseInvalidOperationException("CancelationSource.Dispose: source is not valid.", Internal.GetFormattedStacktrace(1));
            }
        }
