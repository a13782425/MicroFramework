...
namespace MFramework.Core
{
    partial class Internal
    {
        ...
        partial class PromiseRefBase
        {
            ...
            internal abstract class AsyncEnumerableBase<T> : PromiseSingleAwait<bool>, IAsyncEnumerable<T>
            {
                ...
                public virtual AsyncEnumerator<T> GetAsyncEnumerator(int id, CancelationToken cancelationToken)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _enumerableId, newId, id) != id)
                    {
                        throw new PromiseInvalidOperationException("AsyncEnumerable.GetAsyncEnumerator: instance is not valid. AsyncEnumerable may only be used once.", GetFormattedStacktrace(2));
                    }
                ...
                internal T GetCurrent(int id)
                {
                    ...
                    if (_enumerableId != id)
                    {
                        throw new PromiseInvalidOperationException("AsyncEnumerable.GetCurrent: instance is not valid, or the MoveNextAsync operation is still pending.", GetFormattedStacktrace(2));
                    }
#endif
                    return _current;
                }
            ...
            internal abstract class AsyncEnumerableWithIterator<T> : AsyncEnumerableBase<T>
            {
                ...
                internal override Promise<bool> MoveNextAsync(int id)
                {
                    ...
                    if (oldId != id)
                    {
                        ...
                        throw new PromiseInvalidOperationException("AsyncEnumerable.MoveNextAsync: instance is not valid, or the previous MoveNextAsync operation is still pending.", GetFormattedStacktrace(2));
                    }
                    ThrowIfInPool(this);
                    _current = default;
                ...
                internal AsyncStreamYielder<T> YieldAsync(in T value, int id)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _enumerableId, newId, id) != id)
                    {
                        throw new PromiseInvalidOperationException("AsyncStreamWriter.YieldAsync: instance is not valid. This must only be called from the iterator method, and not within any catch or finally blocks.", GetFormattedStacktrace(2));
                    }
                    ThrowIfInPool(this);
                    _current = value;
                    _iteratorCompleteExpectedId = newId;
                ...
                internal override Promise DisposeAsync(int id)
                {
                    ...
                    if (oldId != id)
                    {
                        if (oldId == id + 1)
                        {
                            throw new PromiseInvalidOperationException("AsyncEnumerator.DisposeAsync: the previous MoveNextAsync operation is still pending.", GetFormattedStacktrace(2));
                        }
                ...
                internal override sealed void Handle(PromiseRefBase handler, Promise.State state)
                {
                    ...
                    if (Interlocked.CompareExchange(ref _enumerableId, _iteratorCompleteId, _iteratorCompleteExpectedId) != _iteratorCompleteExpectedId)
                    {
                        ...
                        _rejectContainer = CreateRejectContainer(new PromiseInvalidOperationException("AsyncEnumerable.Create async iterator completed invalidly. Did you YieldAsync without await?"), int.MinValue, null, this);
                ...
                internal void GetResultForAsyncStreamYielder(int enumerableId)
                {
                    ...
                    if (enumId != enumerableId + 1)
                    {
                        ...
                        throw new PromiseInvalidOperationException("AsyncStreamYielder.GetResult: instance is not valid. This should only be called from the async iterator method, and it may only be called once.", GetFormattedStacktrace(2));
                    }
                    // Reset in case the async iterator function completes synchronously from Start.
                    ResetWithoutStacktrace();
                }
                ...
                internal void AwaitOnCompletedForAsyncStreamYielder(PromiseRefBase asyncPromiseRef, int enumerableId, bool hasValue = true)
                {
                    ...
                    {
                        throw new PromiseInvalidOperationException("AsyncStreamYielder: invalid await. Only one await is allowed.", GetFormattedStacktrace(2));
                    }
        ...
        {
            throw new PromiseInvalidOperationException("AsyncEnumerable instance is not valid. AsyncEnumerable may only be used once.", GetFormattedStacktrace(skipFrames + 1));
        }
    } // class Internal
