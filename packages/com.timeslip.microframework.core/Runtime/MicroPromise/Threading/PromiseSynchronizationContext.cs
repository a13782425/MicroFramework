...
namespace MFramework.Core.Threading
{
    /// <summary>
    ...
    public sealed class PromiseSynchronizationContext : SynchronizationContext
    {
        ...
        public PromiseSynchronizationContext(Thread runThread)
        {
            if (runThread == null)
            {
                throw new PromiseArgumentNullException(nameof(runThread), "runThread may not be null", Internal.GetFormattedStacktrace(1));
            }
            _thread = runThread;
