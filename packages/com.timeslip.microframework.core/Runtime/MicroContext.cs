...
namespace MFramework.Core
{
    ...
    public static class MicroContext
    {
        ...
        public static void Launch(MicroRuntimeConfig runtimeConfig)
        {
            ...
            logger.Log("游戏模块开始注册，可以在设置里修改默认注册的模块");
            if (runtimeConfig.AutoRegisterModule)
            {
                Type moduleType = typeof(IMicroModule);
                ...
                foreach (Assembly assembly in assemblies)
                {
                    ...
                    foreach (Type item in types)
                    {
                        if (item.GetCustomAttribute<IgnoreAttribute>() != null)
                            continue;
                        if (!item.IsAbstract && !item.IsInterface && item.IsClass && moduleType.IsAssignableFrom(item))
                        {
                            _isModuleReady = false;
                            //执行模块
                            IMicroModule module = Activator.CreateInstance(item) as IMicroModule;
                            logger.Log($"游戏模块:{item.Name},注册成功");
            ...
            else
            {
                ...
                {
                    if (!modules.TryGetValue(item.AssemblyName, out List<string> list))
                    {
                        list = new List<string>();
                        modules.Add(item.AssemblyName, list);
                    }
                ...
                foreach (Assembly assembly in assemblies)
                {
                    if (!modules.TryGetValue(assembly.FullName, out List<string> list))
                        continue;
                    Type[] types = assembly.GetTypes();
                    foreach (Type item in types)
                    {
        ...
        public static IMicroUpdate AddUpdate(IMicroUpdate microUpdate, Func<bool> onCheckCanUpdate = null)
        {
            ...
            MicroUpdateDescribe describe = MicroObjectPool<MicroUpdateDescribe>.Get();
        ...
        public static IMicroUpdate RemoveUpdate(IMicroUpdate microUpdate)
        {
            ...
            // 回收对象到对象池
            MicroObjectPool<MicroUpdateDescribe>.Recover((MicroUpdateDescribe)describe);
        ...
        public static IMicroLogicUpdate AddLogicUpdate(IMicroLogicUpdate microUpdate, Func<bool> onCheckCanUpdate = null)
        {
            ...
            MicroLogicUpdateDescribe describe = MicroObjectPool<MicroLogicUpdateDescribe>.Get();
            describe.microUpdate = microUpdate;
            describe.onCheckCanUpdateEvent += onCheckCanUpdate;
            // 添加到缓存字典
        ...
        public static IMicroLogicUpdate RemoveLogicUpdate(IMicroLogicUpdate microUpdate)
        {
            ...
            // 回收对象到对象池
            MicroObjectPool<MicroLogicUpdateDescribe>.Recover((MicroLogicUpdateDescribe)describe);
