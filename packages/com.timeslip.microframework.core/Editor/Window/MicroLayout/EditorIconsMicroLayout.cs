...
namespace MFramework.Core.Editor
{
    ...
    /// 内置Icon预览器
    /// </summary>
    internal class EditorIconsMicroLayout : BaseMicroLayout
    {
        public override string Title => "Tool/内置Icon预览器";
        ...
        void SearchGUI()
        {
            using (new GUILayout.HorizontalScope())
            {
                ...
                if (GUILayout.Button(EditorGUIUtility.IconContent("d_winbtn_win_close"), //SVN_DeletedLocal
        ...
        {
            if (float.IsNaN(this.panel.layout.width))
            {
                return;
            }
            ...
            using (var scope = new GUILayout.ScrollViewScope(scroll))
            {
                ...
                var render_width = (this.panel.layout.width / ppp - 13f);
            ...
            using (new GUILayout.HorizontalScope(EditorStyles.helpBox, GUILayout.MaxHeight(viewBigIcons ? 140 : 120)))
            {
                ...
                using (new GUILayout.VerticalScope())
                {
                    ...
                    if (GUILayout.Button("Copy to clipboard", EditorStyles.miniButton))
                    {
                        ...
                        window.ShowNotification(new GUIContent("复制Icon代码成功"), 1);
                    }
                }
